...
def show_false_greyscale(output_raw, width, height, max_iterations):
    """Convert list to array, show using PIL"""
    print(show_false_greyscale.__name__)
    # convert our output to PIL-compatible input
...
def calc_pure_python(draw_output, desired_width, max_iterations):
    ...
    print("Length of x   :", f"{len(x): >12,}")
    print("Total elements:", f"{len(zs): >12,}")
    start_time = time.perf_counter()  # time.time()
    output = calculate_z_serial_purepython(max_iterations, zs, cs)
    end_time = time.perf_counter()  # time.time()
    secs = end_time - start_time
    print(f"{calculate_z_serial_purepython.__name__} took {secs: >8,.3f} secs.")
